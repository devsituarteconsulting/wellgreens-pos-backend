...
@Injectable()
export class SyncHomebaseShiftsService {
  private readonly logger = new Logger(SyncHomebaseShiftsService.name);
  ...
  /**
   * Procesa los shifts por días completos.
   * Fail-fast: si falla una tienda en un día, aborta y lanza error con detalle.
   */
  async syncHomebaseShifts({ from_utc, to_utc }: SyncDto) {
    ...
    this.logger.log(
      `Iniciando sync de shifts por días (fail-fast). Días=${days.length}, Tiendas activas=${totalStores}. Rango: ${from_utc} → ${to_utc}`,
    );
    ...
    for (const day of days) {
      ...
      for (let si = 0; si < totalStores; si++) {
        ...
        try {
          // 3.1) Consulta a Dutchie para el día completo (por fecha de modificacion del shifts)
          const query = {
          ...
          const payload = await this.homebase.getForResolved<any>(s.id, `/locations/${s.home_base_location_uuid}/shifts?page=1&per_page=1000&open=false&with_note=false&date_filter=start_at`, query);
          ...
          await firstValueFrom(
            this.http.post(
              `${this.internalOrigin}/supabase/homebase/shifts`,
              txs,
