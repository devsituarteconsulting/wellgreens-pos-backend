...
import { ReceivedInventoryBackupService } from './backup-received-inventory.service';
...
@Injectable()
export class SyncReceivedInventoryService {
  private readonly logger = new Logger(SyncReceivedInventoryService.name);
  private readonly internalOrigin = process.env.INTERNAL_API_ORIGIN || 'http://localhost:8080'; //TODO: cambiar esto a 8080
  ...
  /**
   * Procesa el inventario recibido por días completos.
   * Fail-fast: si falla una tienda en un día, aborta y lanza error con detalle.
   */
  async syncAllStoresReceivedInventory({ from_utc, to_utc }: SyncDto) {
    ...
    this.logger.log(
      `Iniciando sync de inventario recibido por días (fail-fast). Días=${days.length}, Tiendas activas=${totalStores}. Rango: ${from_utc} → ${to_utc}`,
    );
    ...
    for (const day of days) {
      ...
      for (let si = 0; si < totalStores; si++) {
        ...
        try {
          // 3.1) Consulta a Dutchie para el día completo (por fecha de inventario recibido)
          const query = {
          ...
          await firstValueFrom(
            this.http.post(
              `${this.internalOrigin}/supabase/inventory/receivedinventory`,
              txs,
              { params: { storeId: String(s.id) } }, // tu controller espera storeId (camel)
            ),
          );

          this.logger.log(`[OK][${dayKey}] tienda=${s.name} → reenviadas ${fetchedCount} inventarios`);
        } catch (error: any) {
    ...
    this.logger.log(
      `Iniciando sync de inventario recibido RAW=>NORM`,
    );

    const norm = await this.sb.rpc('upsert_dutchie_received_inventory_norm');
    if (norm.error) {
      norm.error.message = `[upsert_dutchie_received_inventory_norm] ${norm.error.message}`;
      throw norm.error;
    }
    const rowNorm = Array.isArray(norm.data) ? norm.data[0] : norm.data;
    this.logger.log(
      `RAW=>NORM Terminado = affected=${rowNorm?.affected ?? 0} inserted=${rowNorm?.inserted ?? 0} updated=${rowNorm?.updated ?? 0}`,
    );

    this.logger.log(
      `Iniciando sync de inventario recibido NORM=>REVIEW`,
    );
    const review = await this.sb.rpc('upsert_dutchie_received_inventory_review');
    if (review.error) {
      review.error.message = `[upsert_dutchie_received_inventory_review] ${review.error.message}`;
      throw review.error;
    }
    const rowReview = Array.isArray(review.data) ? review.data[0] : review.data;
    this.logger.log(
      `NORM=>REVIEW Terminado = affected=${rowReview?.affected ?? 0} inserted=${rowReview?.inserted ?? 0} updated=${rowReview?.updated ?? 0}`,
    );
