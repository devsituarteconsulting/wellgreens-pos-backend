...
export interface DutchieStore {
  ...
  api_key?: string;
  auth_header: string; // "Basic XXXXX"
}

@Injectable()
export class StoreConfigService {
  private byInternal = new Map<string, DutchieStore>();
  private byDutchie = new Map<string, DutchieStore>();
  ...
  constructor(private readonly cfg: ConfigService) {
    ...
    const arr = JSON.parse(raw) as DutchieStore[];

    for (const s of arr) {
      this.byInternal.set(String(s.id), s);
      this.byDutchie.set(String(s.dutchie_store_id), s);
    }
  ...
  resolve(storeId?: string): DutchieStore {
