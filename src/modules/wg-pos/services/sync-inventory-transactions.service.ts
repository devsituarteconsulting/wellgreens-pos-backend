...
@Injectable()
export class SyncInventoryTransactionsService {
  private readonly logger = new Logger(SyncInventoryTransactionsService.name);
  ...
  /**
   * Procesa los inventory transaction por días completos.
   * Fail-fast: si falla una tienda en un día, aborta y lanza error con detalle.
   */
  async syncInventoryTransaction({ from_utc, to_utc }: SyncDto) {
    ...
    this.logger.log(
      `Iniciando sync de inventory transactions por días (fail-fast). Días=${days.length}, Tiendas activas=${totalStores}. Rango: ${from_utc} → ${to_utc}`,
    );
    ...
    for (const day of days) {
      ...
      for (let si = 0; si < totalStores; si++) {
        ...
        try {
          // 3.1) Consulta a Dutchie para el día completo (por fecha de modificacion del inventory transaction)
          const query = {
          ...
          await firstValueFrom(
            this.http.post(
              `${this.internalOrigin}/supabase/inventory/inventorytransaction`,
              txs,
              { params: { storeId: String(s.id) } },
            ),
          );

          this.logger.log(`[OK][${dayKey}] tienda=${s.name} → reenviadas ${fetchedCount} inventory transaction`);
        } catch (error: any) {
