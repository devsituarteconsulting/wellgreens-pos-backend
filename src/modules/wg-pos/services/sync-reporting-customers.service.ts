...
@Injectable()
export class SyncReportingCustomersService {
  private readonly logger = new Logger(SyncReportingCustomersService.name);
  ...
  /**
   * Procesa los clientes por días completos.
   * Fail-fast: si falla una tienda en un día, aborta y lanza error con detalle.
   */
  async syncReportingCustomers({ from_utc, to_utc }: SyncDto) {
    ...
    this.logger.log(
      `Iniciando sync de clientes por días (fail-fast). Días=${days.length}, Tiendas activas=${totalStores}. Rango: ${from_utc} → ${to_utc}`,
    );
    ...
    for (const day of days) {
      ...
      for (let si = 0; si < totalStores; si++) {
        ...
        try {
          // 3.1) Consulta a Dutchie para el día completo (por fecha de modificacion del cliente)
          const query = {
          ...
          await firstValueFrom(
            this.http.post(
              `${this.internalOrigin}/supabase/reporting/customers`,
              txs,
            ),
          );

          this.logger.log(`[OK][${dayKey}] tienda=${s.name} → reenviadas ${fetchedCount} clientes`);
        } catch (error: any) {
