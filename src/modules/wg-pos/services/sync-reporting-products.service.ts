...
@Injectable()
export class SyncReportingProductsService {
  private readonly logger = new Logger(SyncReportingProductsService.name);
  ...
  /**
   * Procesa los products por días completos.
   * Fail-fast: si falla una tienda en un día, aborta y lanza error con detalle.
   */
  async syncAllStoresProducts({ from_utc , to_utc}: SyncDto) {
    ...
    this.logger.log(
      `Iniciando sync de productos por días (fail-fast). Tiendas activas=${totalStores}. Rango: ${from_utc}  a ${to_utc}`,
    );
    ...
    for (const day of days) {
      ...
      for (let si = 0; si < totalStores; si++) {
        ...
        this.logger.log(
          `[${from}] [${storeIdx}/${totalStores}] tienda=${s.name} (#${s.id}, dutchie=${s.dutchie_store_id}) | faltan=${remaining}`,
        );

        try {
          // 3.1) Consulta a Dutchie para el día completo (por fecha de producto)
          const query = {
          ...
          await firstValueFrom(
            this.http.post(
              `${this.internalOrigin}/supabase/reporting/products`,
              txs,
              { params: { storeId: String(s.id) } }, // tu controller espera storeId (camel)
            ),
          );

          this.logger.log(`[OK][${from}] tienda=${s.name} → reenviadas ${fetchedCount} productos`);
        } catch (error: any) {
          ...
          this.logger.error(`[FAIL-FAST][${from}] tienda=${s.name} → ${errText}`);
          this.logger.log(`....`);
          this.logger.log(`${from} Terminado con ERROR ============`);

          // Lanza error con detalle de día y tienda
          throw new Error(`Falló el proceso en el día ${from}, tienda "${s.name}" (id=${s.id}). Detalle: ${errText}`);
        }
      }

      // Log fin de día
      this.logger.log(`....`);
      this.logger.log(`${from} Terminado ============`);
    }
