...
@Injectable()
export class SyncInventorySnapshotService {
  private readonly logger = new Logger(SyncInventorySnapshotService.name);
  ...
  /**
   * Procesa los inventory snapshot por días completos.
   * Fail-fast: si falla una tienda en un día, aborta y lanza error con detalle.
   */
  async syncInventorySnapshot({ from_utc, to_utc }: SyncDto) {
    ...
    this.logger.log(
      `Iniciando sync de inventory snapshot por días (fail-fast). Días=${days.length}, Tiendas activas=${totalStores}. Rango: ${from_utc} → ${to_utc}`,
    );
    ...
    for (const day of days) {
      ...
      for (let si = 0; si < totalStores; si++) {
        ...
        try {
          // 3.1) Consulta a Dutchie para el día completo (por fecha de modificacion del inventory snapshot)
          const query = {
          ...
          await firstValueFrom(
            this.http.post(
              `${this.internalOrigin}/supabase/inventory/snapshots`,
              txs,
              { params: { storeId: String(s.id) } },
            ),
          );

          this.logger.log(`[OK][${dayKey}] tienda=${s.name} → reenviadas ${fetchedCount} inventory snapshot`);
        } catch (error: any) {
